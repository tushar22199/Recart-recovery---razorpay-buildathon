...
export default function RecoveryPage() {
  ...
  return <AppShell>
    ...
    <QueryState loading={summaryQuery.isLoading} error={false}><div className="metrics-grid">
      ...
      <section className="panel attempts-panel animate-rise-in animate-delay-1"><div className="panel-head"><div><div className="eyebrow">Needs a nudge</div><h2>Recovery attempts</h2></div><div className="panel-head-actions"><div className="search-box"><Search size={15} /><input data-testid="input-search-attempts" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search attempts" /></div><button data-testid="button-filter-attempts" className="icon-button bordered" title="Filter attempts"><Filter size={15} /></button></div></div><div className="table-meta"><span>{filteredAttempts.length} recovery attempts</span><span className="meta-rule" /><span className="meta-muted">Sorted by risk</span></div><QueryState loading={attemptsQuery.isLoading} error={attemptsQuery.isError} empty={!attemptsQuery.isLoading && !attemptsQuery.isError && attempts.length === 0} onRetry={() => void attemptsQuery.refetch()}><AttemptsTable
        attempts={attempts}
