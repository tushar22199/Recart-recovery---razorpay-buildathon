...
export default function AttemptDetailPage() {
  ...
  const query = useGetRecoveryAttempt(id, {
    query: {
      queryKey: getGetRecoveryAttemptQueryKey(id),
      enabled: Boolean(id),
    },
  });
  ...
  const canRetry = Boolean(
    attempt &&
      !["recovered", "escalated", "closed"].includes(
    ...
    retry.mutate(
      { id },
      {
        onSuccess: () => {
          ...
          void queryClient.invalidateQueries({
            queryKey: getGetRecoveryAttemptsQueryKey(),
          });
          void queryClient.invalidateQueries({
            queryKey: getGetRecoverySummaryQueryKey(),
          });
          void queryClient.invalidateQueries({
            queryKey: getGetRecoveryActivityQueryKey(),
          });
        },
      },
    );
    ...
    <AppShell>
      <div className="detail-page">
        ...
        >
          {attempt && (
            <>
              <div className="detail-hero animate-rise-in">
                <div>
                  ...
                  <h1>{attempt.customer}</h1>

                  <div className="hero-sub">
                    <span>{attempt.email}</span>
                    <span className="sub-separator" />
                    <span>
                      Detected {shortDate(attempt.detectedAt)}
                    </span>
                  </div>
                </div>
                ...
                <div className="detail-actions">
                  ...
                  <ActionButton
                    ...
                    disabled={!canRetry}
                  >
                    {canRetry ? "Retry recovery" : "No retry available"}
                  </ActionButton>
                </div>
              </div>

              {retry.isError && (
                <ErrorNotice text="The retry could not be queued. No changes were made." />
              )}

              <div className="detail-layout">
                <div className="detail-main">
                  <section className="panel value-panel animate-rise-in animate-delay-1">
                    <div className="value-panel-main">
                      <div className="eyebrow">Payment at risk</div>

                      <div
                        data-testid="text-attempt-amount"
                      ...
                      <div className="value-caption">
                        {attempt.paymentMethod} · {attempt.channel}
                      </div>
                    </div>
                    ...
                    <div className="value-side">
                      <div>
                        ...
                        <strong>{attempt.failureReason}</strong>
                      ...
                      <div>
                        ...
                        <strong>
                          {attempt.expiresAt
                            ? `Ends ${shortDate(attempt.expiresAt)}`
                            : "No expiry set"}
                        </strong>
                      </div>
                    </div>
                  ...
                  {attempt.decision && (
                    <section className="panel decision-panel animate-rise-in animate-delay-2">
                      ...
                      <div className="decision-explanation">
                        ...
                        {attempt.decision.shouldRecover &&
                          !attempt.decision.shouldEscalate && (
                            <div className="decision-summary">
                              ...
                              <strong>
                                {attempt.decision.incentivePercent}% incentive
                              </strong>{" "}
                              and a{" "}
                              <strong>
                                {attempt.decision.delayMinutes} min delay
                              </strong>
                              .
                            </div>
                          )}
                        ...
                        {attempt.decision.shouldEscalate && (
                          <div className="decision-summary">
                            The attempt requires human follow-up instead of
                            automated recovery.
                          </div>
                        )}
                      </div>
                      ...
                      <div className="decision-grid">
                        ...
                        <div>
                          <span>Risk</span>
                          <strong
                            className={`risk-${attempt.decision.riskLevel.toLowerCase()}`}
                          >
                        ...
                        <div>
                          ...
                          <strong>
                            {Math.round(
                              attempt.decision.confidence * 100,
                            )}
                            %
                          </strong>
                      ...
                      <div className="decision-grid execution-grid">
                        ...
                        <div>
                          <span>Delay</span>
                          <strong>
                            {attempt.decision.delayMinutes} min
                          </strong>
                        </div>
                        ...
                        <div>
                          <span>Incentive</span>
                          <strong>
                            {attempt.decision.incentivePercent}%
                          </strong>
                        </div>
                      </div>
                  ...
                  {attempt.status.toLowerCase() === "recovered" && (
                    <section className="panel outcome-panel animate-rise-in animate-delay-2">
                      ...
                      <p className="outcome-message">
                        Payment captured successfully through the recovery
                        payment link.
                      </p>
                  ...
                  <section className="panel audit-panel animate-rise-in animate-delay-2">
                    <div className="panel-head">
                      <div>
                        <div className="eyebrow">Immutable record</div>
                        <h2>Audit trail</h2>
                      </div>

                      <button
                        data-testid="button-copy-attempt-id"
                        className="button button-quiet"
                        onClick={() =>
                          void navigator.clipboard?.writeText(attempt.id)
                        }
                      >
                ...
                <aside className="detail-side">
                  <section className="side-card animate-rise-in animate-delay-2">
                    ...
                    <div className="side-person">
                      <div className="large-avatar">
                        {attempt.customer.slice(0, 1)}
                      </div>

                      <div>
                        <strong>{attempt.customer}</strong>
                        <span>{attempt.email}</span>
                      </div>
                    </div>
                    ...
                    <div className="side-details">
                      ...
                      <div>
                        ...
                        <strong>{attempt.lastAction}</strong>
                        <small>
                          {shortDate(attempt.lastActionAt)}
                        </small>
                      </div>
                    </div>

                    <div className="side-contact-action">
                      {attempt.channel.toLowerCase() === "whatsapp" ? (
                        <button
                          data-testid="button-contact-customer"
                          className="button button-outline w-full"
                          disabled
                        >
                          <MessageCircle size={14} /> WhatsApp notification
                          sent
                        </button>
                      ) : (
                        <button
                          ...
                          onClick={async () => {
                            try {
                              ...
                              window.alert(
                                `Customer email copied: ${attempt.email}`,
                              );
                            } catch {
                              window.alert(
                        ...
                        >
                          <Mail size={14} /> Copy customer email
                        </button>
                      )}
                    </div>
                  </section>

                  <section className="bounded-note animate-rise-in animate-delay-3">
                    <div className="bounded-note-icon">
                      <ShieldCheck size={18} />
                    </div>

                    <div>
                      <strong>Bounded by guardrails</strong>

                      <p>
                        {attempt.status.toLowerCase() === "recovered"
                          ? "Recovery completed successfully. No further automated actions are required."
                          : attempt.status.toLowerCase() === "escalated"
                            ? "Automation has stopped. Human follow-up is required."
                            : `This attempt can run ${Math.max(
                                0,
                                attempt.maxAttempts - attempt.attempts,
                              )} more time${
                                attempt.maxAttempts -
                                  attempt.attempts ===
                                1
                      ...
                      <Link
                        href="/settings"
                        data-testid="link-view-guardrails"
